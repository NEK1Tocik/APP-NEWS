...
class CategoryDetailView(generics.RetrieveUpdateDestroyAPIView):
    ...
    serializer_class = CategorySerializer
    permission_classes = [permissions.IsAuthenticatedOrReadOnly]
    lookup_field = 'slug'     # использование slug для того, чтобы перейти на страницу конкретной категории
...
class PostListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        return queryset     # возвращаем посты с учётом прав доступа человека
...
class PostDetailView(generics.RetrieveUpdateDestroyAPIView):
    ...
    queryset = Post.objects.select_related('author', 'category')     # достаём из БД конкретные объекты формата "POST"
    ...
    def retrieve(self, request, *args, **kwargs):
        """Увеличивает счётчик просмотров при GET запросе"""
...
def popular_posts(request):
    ...
    return Response(serializer.data)     # возвращаем массив данных всех постов, которых человек будет видеть
